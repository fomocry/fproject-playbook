# ⚡ Fonctionnalités

Tour complet de ce que l'extension fait, feature par feature.

## 🎯 Quick Buy dans les feeds X & Discord

La feature signature. Quand un contract address Solana apparaît dans ta timeline X ou dans un salon Discord, l'extension **injecte les boutons de buy directement dans le feed**, à côté du CA.

```
┌─ Tweet ───────────────────────────────┐
│ @caller : nouveau play 👀             │
│ 9ki4MjbQ...icpump                     │
│                                       │
│ ⚡ F Project : [P1] [P2] [P3]         │
└───────────────────────────────────────┘
```

Tu vois le CA → tu cliques sur un preset → le buy part sur ton wallet actif. Tu n'as jamais quitté ton feed.

> ⚠️ Un clic = un buy réel. Les presets exécutent immédiatement avec le montant configuré. Configure tes presets avant de scroller (voir [🎛 Presets](presets.md)).

## 🔍 CA auto-rempli sur les pages token

Quand tu ouvres une page token (Axiom, GMGN, padre, page PumpFun…), l'extension **détecte le contract address automatiquement** et le charge dans le panneau. Plus de copier-coller, plus de risque de coller le mauvais CA.

Le token détecté s'affiche dans l'extension avec ses boutons de buy prêts à tirer.

## 👛 Multi-wallet : exécution simultanée

Tu sélectionnes tes wallets dans l'extension, et chaque buy part **sur tous les wallets sélectionnés en même temps**. Une entrée répartie sur 3 wallets = un seul clic, trois exécutions simultanées.

Les onglets Positions et Trackers se filtrent aussi selon les wallets sélectionnés — tu vois exactement ce qui concerne les wallets avec lesquels tu joues.

## 💸 Panneau de vente par wallet

Après un buy, un **panel de sell apparaît pour chaque wallet sélectionné**. Si tu détiens le même token sur plusieurs wallets, tu gères chaque sortie indépendamment :

```
Wallet 1 :  0.8 SOL  +42%   [25%] [50%] [100%]
Wallet 2 :  0.3 SOL  +42%   [25%] [50%] [100%]
```

Pratique pour sortir tes initials sur un wallet tout en laissant courir l'autre.

## 📋 Onglet Positions

Toutes tes positions ouvertes, les mêmes que sur le bot, visibles depuis n'importe quelle page web : token, entrée, P&L, boutons de vente rapide. Tu n'as plus besoin d'ouvrir Telegram pour surveiller ou couper une position.

## 👁 Onglet Trackers

Tes ruggers suivis, avec leur config active. Tu peux vérifier d'un coup d'œil qui est tracké et avec quels réglages, sans quitter ta navigation.

> 💡 Workflow type : le bot tourne en autonome sur tes ruggers trackés, et l'extension te sert pour les entrées manuelles opportunistes — un call qui passe dans un feed, un token repéré sur une page. Les deux se complètent.

Pages liées :
- [🧩 Vue d'ensemble & Installation](overview.md)
- [🎛 Presets P1 / P2 / P3](presets.md)
- [📊 Positions](../outils/positions.md)
- [⚡ Quick Buy by CA](../outils/quick-buy-ca.md)
