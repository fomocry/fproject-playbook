...
Envoyée quand un rugger en mode New Token Creations détecte une création de token. Cette notification inclut un bouton **📋 Copier le CA** qui permet de copier l'adresse complète du contrat en un clic. L'adresse est affichée tronquée dans le message pour la lisibilité, mais le bouton copie l'adresse entière.
...
### 🛑 Protection Auto-Achat déclenchée

Envoyée quand le bot désactive automatiquement l'auto-achat d'un rugger après avoir atteint le seuil de pertes consécutives. Le message indique quel rugger a été désactivé et combien de pertes consécutives ont été enregistrées.
...
> ⚠️ **Important** : La PnL Card ne se génère que quand **100% de la position** est vendue. Si vous avez 3 niveaux de TP et que seuls 2 se sont déclenchés, la PnL Card ne se génèrera qu'une fois le dernier TP atteint ou la position entièrement vendue.
